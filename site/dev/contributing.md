...
pnpm dev:core         # `@tg-wagmi/core` playground
pnpm dev:create-wagmi # `create-wagmi` cli tool
pnpm dev:cli          # `@tg-wagmi/cli` tool
pnpm dev:next         # `wagmi` playground with Next.js
pnpm dev:nuxt         # `@tg-wagmi/vue` playground with Nuxt.js
pnpm dev:react        # `wagmi` playground (same as `pnpm dev`)
pnpm dev:vue          # `@tg-wagmi/vue` playground
```
...
- `pnpm test:core` — runs `@tg-wagmi/core` tests
- `pnpm test:react` — runs `wagmi` tests
- `pnpm test:vue` — runs `@tg-wagmi/vue` tests
