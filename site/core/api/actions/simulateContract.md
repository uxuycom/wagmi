...
```ts
import { type SimulateContractParameters } from '@tg-wagmi/core'
```
...
::: code-group
```ts [index.ts]
import { getAccount, simulateContract } from '@tg-wagmi/core'
import { abi } from './abi'
import { config } from './config'
...
```ts
import { type SimulateContractReturnType } from '@tg-wagmi/core'
```
...
```ts
import { type SimulateContractErrorType } from '@tg-wagmi/core'
```
